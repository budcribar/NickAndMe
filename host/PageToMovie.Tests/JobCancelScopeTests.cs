...
public class JobCancelScopeTests
{
    ...
    public void JobStore_TryCancel_marks_running_job_cancelled()
    {
        ...
        var rec = store.Create(new PageToMovie.Core.Models.JobRecord
        {
            UserId = "alice",
            Status = "running",
            Kind = "scene",
    ...
    {
        var store = new JobStore();
        store.Create(new PageToMovie.Core.Models.JobRecord { UserId = "alice", Status = "running" });
        store.Create(new PageToMovie.Core.Models.JobRecord { UserId = "bob", Status = "running" });
