...
public class FfmpegProcessHangTests
{
    private static string? FindFfmpeg()
    {
        ...
        var eng = Path.GetFullPath(Path.Combine(
            ...
            "FilmStudio.Engine", "bin", "Debug", "net10.0", "Resources", "ffmpeg.exe"));
        if (File.Exists(eng)) return eng;

        var api = Path.GetFullPath(Path.Combine(
            AppContext.BaseDirectory, "..", "..", "..", "..",
            "FilmStudio.Api", "bin", "Debug", "net10.0", "Resources", "ffmpeg.exe"));
        ...
        foreach (var c in new[]
                 {
                     ...
                     Path.GetFullPath(Path.Combine(
                         AppContext.BaseDirectory, "..", "..", "..", "..",
                         "FilmStudio.Fakes", "Fixtures", "clip_merge_10s.mp4")),
                     Path.GetFullPath(Path.Combine(
                         AppContext.BaseDirectory, "..", "..", "..", "..", "..",
